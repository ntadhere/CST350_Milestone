...
namespace CST350_Milestone.Services.Business
{
    ...
    {
        public bool SaveGameState(int userId, string gameStateJson)
        {
            // Connection string (adjust as needed)
            string connectionString = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=MilestoneUser;Integrated Security=True;Connect Timeout=30";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO Games (UserId, GameState) VALUES (@UserId, @GameState)";

                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@UserId", userId);
                        command.Parameters.AddWithValue("@GameState", gameStateJson);
